...
//
// Melhoria "Estoque de filamento por custo médio ponderado": deixou de
// existir "rolo" -- cadastrar um filamento NOVO já É a primeira compra
// (peso comprado em kg + valor total pago), exatamente como
// Acessório/Insumo. "Repor estoque" de uma cor já cadastrada vira
// RestockForm.tsx (uma FilamentPurchase nova, soma no estoque existente e
// recalcula a média) -- não mais uma submissão deste formulário com campos
// pré-preenchidos/travados. Editar aqui só corrige marca/material/cor
// (nunca estoque/custo, que só mudam por uma compra real).
export function FilamentForm({
  ...
  const price = parseFloat(totalCost)
  const hasWeightAndPrice = weightKg !== '' && totalCost !== '' && !isNaN(weight) && !isNaN(price) && weight > 0 && price > 0
    ...
    >
      <form ref={formRef} action={action} className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 sm:p-5">
        ...
        <div className="col-span-full mt-1 flex items-center justify-end gap-3">
          ...
          <SubmitButton pendingLabel="Salvando…">{editingFilament ? 'Salvar alterações' : 'Cadastrar filamento'}</SubmitButton>
        </div>
      </form>
    </dialog>
  )
