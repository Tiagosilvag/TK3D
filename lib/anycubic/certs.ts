// Certificado/chave TLS cliente fixos usados no handshake mTLS do MQTT da
// Anycubic (públicos, extraídos por engenharia reversa comunitária -- ver
// spec 2026-09-12 §2.1 -- não são segredo do usuário). Embutidos como
// string em vez de arquivo lido em runtime (fs.readFileSync) porque o
// build do Next.js empacota o código do servidor em chunks e não copia
// arquivos estáticos referenciados assim pro lugar certo -- bug real
// encontrado em produção (ENOENT em .next/server/chunks/certs/...).
export const ANYCUBIC_MQTT_CA_CERT = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`

export const ANYCUBIC_MQTT_CLIENT_CERT = `-----BEGIN CERTIFICATE-----

i6baCTeifwp5Jpdzv4imcCPvayKUNuX32vYNfNkWC/R5
-----END CERTIFICATE-----
`

export const ANYCUBIC_MQTT_CLIENT_KEY = `-----BEGIN PRIVATE KEY-----

4swn72uwBRlhjAw46MF77SBQ
-----END PRIVATE KEY-----
`
